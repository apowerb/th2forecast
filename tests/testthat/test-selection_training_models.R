...
test_that("th2_mars_engine returns a MARS model", {
  model_mars <- th2_mars_engine(data, "value", "date", engine="earth", mars_features="month")
  expect_is(model_mars, "workflow")

  expect_warning(th2_mars_engine(data, "valor", "date", engine="earth", mars_features="month"))
})
...
test_that("model_selection_train returns a modeltime table", {
  ...
  # Test outputs
  model_table <- model_selection_train(data, c("prophet", "lr", "mars"), "value", "date")

  # Test for classes of model_table
  expect_s3_class(model_table, c('mdl_time_tbl', 'tbl_df', 'tbl', 'data.frame'))

  # Test errors
  expect_warning(model_selection_train(data, c("prophet", "lr", "mars"), "valores", "date"))
  expect_warning(model_selection_train(data, c(), "value", "date"))
  expect_warning(model_selection_train(data, 123, "value", "date"))
  expect_warning(model_selection_train(data, c("profeta", "lr", "marte"), "value", "date"))

  dataframe_test <- data.frame()
  expect_warning(model_selection_train(dataframe_test, c("prophet", "lr", "mars"), "value", "date"))
